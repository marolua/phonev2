...
const normalizeCompany = (company, index = 0) => ({
    ...
    description: company?.description || 'Contactez cette entreprise pour plus d’informations.',
...
const normalizeMessage = (message, index = 0) => ({
    ...
    time: message?.time || new Intl.DateTimeFormat('fr-FR', { hour: '2-digit', minute: '2-digit' }).format(Date.now()),
...
export const useServicesDirectory = (initialCompanies = []) => {
    ...
    const companies = ref((initialCompanies.length ? initialCompanies : demoCompanies).map(normalizeCompany));
    ...
    const loadCompanies = async () => {
        ...
        if (Array.isArray(remoteCompanies)) companies.value = remoteCompanies.map(normalizeCompany);
