...
const demoArticles = [
  {
    ...
    excerpt:
      "Les équipes météo annoncent un changement de temps spectaculaire sur toute la ville.",
    content:
      "Les premières lueurs du jour ont laissé place à un ciel chargé au-dessus de Los Santos. Les services de la ville invitent les habitants à rester attentifs aux prochaines alertes et à adapter leurs déplacements.",
    category: "Actualités",
    author: "Maya Brooks",
  ...
  },
  {
    ...
    excerpt:
      "Créateurs locaux, produits frais et musique live seront au rendez-vous ce week-end.",
    content:
      "Le front de mer accueillera dès samedi un marché réunissant artisans, restaurateurs et producteurs de la région.",
    category: "Vie locale",
    author: "Noah Carter",
  ...
  },
  {
    ...
    excerpt:
      "Un dispositif spécial est déployé dans les quartiers les plus fréquentés.",
    content:
      "La mairie et les services d’urgence coordonnent leurs équipes pour accompagner les rassemblements prévus cette semaine.",
    category: "Sécurité",
    author: "Maya Brooks",
  ...
  },
  {
    ...
    excerpt:
      "Les clubs de la ville se retrouvent pour une rencontre placée sous le signe du fair-play.",
    content:
      "Les associations sportives de Los Santos préparent une soirée ouverte à tous les habitants.",
    category: "Sport",
    author: "Alex Reed",
...
const normalizeArticle = (article, index = 0) => ({
  ...
  excerpt:
    article?.excerpt?.trim() || article?.content?.trim()?.slice(0, 120) || "",
...
export const useNewsFeed = (initialArticles = []) => {
  ...
  const articles = ref(
    (initialArticles.length ? initialArticles : demoArticles).map(
      normalizeArticle,
    ),
  );
  ...
  const loadArticles = async () => {
    ...
    const remoteArticles = Array.isArray(response)
      ? response
      : response?.articles;
